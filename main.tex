...
through multi-turn consultation, discrimination rises to $0.75$/$0.80$. A causal manipulation identifies what the signal
is: removing, restoring, or requiring the model to request the single discriminating finding changes
accuracy by $9.1$ points and the committed diagnosis on $36\%$ of cases, yet leaves discrimination
unchanged across all three conditions. Confidence instead separates episodes in which the model
obtained that finding from those in which it did not ($\mathrm{AUC}=0.661$), and discriminates
correctness \emph{better} among episodes where it was missing. Elicited confidence therefore reports
...
\begin{table}[t]
\caption{Causal manipulation of the evidence available to the model, \texttt{gpt-5.4-mini}, 214
AgentClinic cases $\times$ 2 seeds. The single key discriminating finding is removed, restored, or
made available only on request. The manipulation works---restoring it raises accuracy by $9.1$
points and changes the committed diagnosis on $36.4\%$ of paired cases---yet discrimination is
...
\midrule
\multicolumn{6}{l}{\emph{Pairwise differences (bootstrap)}}\\
Given $-$ Withheld & & & & & $+.006$ [$-.081$, $+.094$] \\
Acquired $-$ Given & & & & & $+.039$ [$-.039$, $+.118$] \\
Acquired $-$ Withheld & & & & & $+.044$ [$-.026$, $+.117$] \\
\bottomrule
\end{tabular}}
\end{table}
