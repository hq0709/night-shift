...
can't-miss diagnosis was itself missed remains intact (AUROC $0.687$/$0.667$), so a single
calibration figure describes neither endpoint. When the
...
interactive deployment. A gate validated on standard benchmark items does not transfer to
high-acuity presentations or across modality.
\end{abstract}
...
\emph{A gate must be validated on the acuity stratum it will protect.} Validation on standard
