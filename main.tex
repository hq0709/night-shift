...
\title{Confidence Reports an Information State, Not an Answer State:\\
Why Verbalized Uncertainty Fails Where Clinical Errors Are Not Informational}

\author{Anonymous
\thanks{Manuscript submitted 2026. Corresponding author: --.}}

\markboth{IEEE Transactions on Biomedical and Health Informatics}%
{Author \MakeLowercase{\textit{et al.}}: Confidence Reports an Information State}
...
through multi-turn consultation, discrimination rises to $0.75$/$0.80$. A causal manipulation, replicated across two models and two task
types, identifies what the signal is. Removing, restoring, or requiring the model to request the
single decisive item changes accuracy by $9$ points on diagnostic vignettes and by $41$ points on
calculations---where removal makes the item unanswerable---and moves stated confidence by $3.6$
points, yet leaves discrimination statistically unchanged throughout. Confidence instead separates episodes in which the model obtained
that item from those in which it did not ($\mathrm{AUC}=0.661$), and discriminates correctness
\emph{better} among episodes where it was missing. Only evidence \emph{agency}---the model having
obtained the item through its own action---adds discrimination, and only on the stronger model
($+0.107$). Elicited confidence therefore reports
an information state, not an answer state; it is informative exactly where insufficient information
is the dominant error mode and uninformative where errors are failures of knowledge. The practical
consequence is quantified: a deferral threshold meeting an $0.85$ service level in validation returns
$0.716$--$0.763$ when transferred unchanged to can't-miss presentations, with nothing in the
validation data signalling the shortfall.
\end{abstract}
...
\begin{table}[t]
\caption{Causal manipulation of the evidence available to the model, replicated across two models
and two task types. In AgentClinic the removed item is the key diagnostic finding; in MedCalc it is
a numerically required computational input, whose removal makes the item unanswerable rather than
merely harder. Every manipulation moves accuracy and stated confidence substantially; none moves
discrimination, with the single exception of evidence \emph{agency} on the stronger model.}
\label{tab:evidence}
\centering
\small
\resizebox{\columnwidth}{!}{%
...
\toprule
Condition & $n$ & Acc. & Conf. & Key obtained & AUROC [95\% CI] \\
\midrule
Withheld & 428 & .764 & 8.96 & .000 & .727 [.669, .782] \\
Given & 428 & .855 & 9.43 & 1.000 & .732 [.661, .798] \\
Acquired on request & 428 & .629 & 8.56 & .463 & .769 [.723, .814] \\
\midrule
\midrule
\multicolumn{6}{l}{\emph{\texttt{gpt-5.4}, same cases}}\\
Withheld & 428 & .811 & 8.81 & .000 & .755 [.691, .815] \\
Given & 428 & .897 & 9.36 & 1.000 & .693 [.606, .777] \\
Acquired on request & 428 & .736 & 8.36 & .489 & .801 [.750, .850] \\
\midrule
\multicolumn{6}{l}{\emph{MedCalc: a required computational input removed}}\\
Withheld & 467 & .302 & 5.72 & .000 & .728 [.681, .774] \\
Given & 466 & .712 & 9.30 & 1.000 & .792 [.746, .835] \\
\midrule
\multicolumn{6}{l}{\emph{Pairwise differences in AUROC (bootstrap)}}\\
AgentClinic, mini: Given $-$ Withheld & & & & & $+.006$ [$-.081$, $+.094$] \\
AgentClinic, gpt-5.4: Given $-$ Withheld & & & & & $-.062$ [$-.168$, $+.041$] \\
MedCalc, mini: Given $-$ Withheld & & & & & $+.062$ [$-.001$, $+.126$] \\
AgentClinic, gpt-5.4: Acquired $-$ Given & & & & & $\mathbf{+.107}$ [$+.008$, $+.204$] \\
\bottomrule
\end{tabular}}
\end{table}

\subsection{Evidence availability does not create the signal}
The account that suggests itself for the interactive result---that a model which has obtained the
discriminating finding converges for a reason related to correctness, so its confidence inherits
that reason---is testable directly, and it is wrong. We hold the case, the reference label, the
prompt and the schema fixed and vary only what the model has in hand: the key discriminating finding
is removed from the vignette, restored to it, or made available only if the model requests it.

The manipulation is effective. Restoring the finding raises accuracy from $0.764$ to $0.855$, raises
mean stated confidence from $8.96$ to $9.43$, and changes the committed diagnosis on $36.4\%$ of
paired cases, so the model is demonstrably using it. Discrimination, however, does not move:
$0.727$, $0.732$ and $0.769$ across the three conditions, with every pairwise interval containing
zero (Table~\ref{tab:evidence}). Making the decisive evidence available does not make elicited
confidence better at separating right answers from wrong ones, and neither does requiring the model
to obtain it itself.


The result replicates on \texttt{gpt-5.4} over the same cases: supplying the key finding raises
accuracy from $0.811$ to $0.897$ while discrimination does not improve ($-0.062$
$[-0.168,+0.041]$). On that model the agency contrast is significant, however---acquiring the
finding through the model's own action rather than receiving it yields $+0.107$ $[+0.008,+0.204]$---so
what the model did, rather than what it was given, carries some information about correctness for
the stronger model. The weaker model shows the same sign at half the size and without significance.

The sharpest version of the manipulation is on MedCalc, where the removed item is a numerically
required input rather than a diagnostic clue, and its removal makes the calculation unanswerable
rather than merely harder. Accuracy falls from $0.712$ to $0.302$ and mean stated confidence from
$9.30$ to $5.72$: the model registers the deficiency clearly and reports it. Discrimination moves by
$+0.062$ $[-0.001,+0.126]$, which does not exclude zero. A signal that responds to a $3.6$-point
drop in available information while barely responding to a $41$-point drop in accuracy is measuring
the former, not the latter---which is the claim of this section stated as directly as the data
permit.
...
state. It tracks, imperfectly, whether the model has what it needs; it does not track whether the
conclusion it drew is correct. The two coincide only when insufficiency is the dominant source of
error---which is the case in interactive consultation, where the model may simply not have asked, and
is not the case at the capability frontier, where the information is fully present and the error is
one of knowledge. This single account predicts all four observations: the frontier collapse, the
interactive recovery, the null under evidence manipulation, and the inversion in the
not-obtained subgroup.
...
confidence without changing discrimination at all (Section~V-I).
